...
public class ContasAReceberServiceTest {
    ...
    void testeCadastrarContasAReceberEstaFuncionando() {
        ...
        Mockito.when(iContasAReceberRepository.existsById(Mockito.anyLong())).thenReturn(true);
    ...
    void testeBuscarContasAReceberViaId() {
        Optional<ContasReceberModel> contasReceberModelOptional = contasAReceberService.exibirRecebimentoViaId(1L);
    ...
    void testeDeletarContasAReceberViaId() {
        contasAReceberService.deletarContasCadastradas(1L);
        Assertions.assertTrue(true);
    }
}
